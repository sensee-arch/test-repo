...
- 实现一个单文件 Web Todo List 应用（HTML/CSS/JS + localStorage）
- 遵守零外部依赖、纯前端、单文件交付的约束
- 所有代码变更通过 git 提交到 `flyinghub-*` 分支，必须推送至远程
...
- **幂等性保证**: `saveTodos()` 是全量覆盖写入；`toggleTodo(id)` 是幂等的
- **明确禁止**:
...
- **运行环境要求**: 仅浏览器（Chrome / Firefox / Edge ≥ 90），无需 Node.js、Python 或其他后端运行时
- **启动入口**: 浏览器打开 `src/web/todo/index.html` 即可运行
- **本地开发与调试流程**:
  1. 任意 Agent 修改代码前必须先 `git checkout` 到正确的分支
